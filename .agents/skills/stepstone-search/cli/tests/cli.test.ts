...
test("missing APIFY_TOKEN errors with NO_TOKEN", async () => {
  const { stderr, exitCode } = await runCLI(["search", "--query", "developer"], {
    ...process.env,
    APIFY_TOKEN: "",
  })
...
// Live test — only runs when a real APIFY_TOKEN is present (costs Apify credits).
const live = process.env.APIFY_TOKEN ? test : test.skip
live("live: search returns normalized jobs", async () => {
  const { stdout, exitCode } = await runCLI(["search", "--query", "software engineer", "--limit", "3", "--format", "json"])
  expect(exitCode).toBe(0)
  const parsed = JSON.parse(stdout)
  expect(parsed.meta.source).toBe("stepstone")
  if (parsed.jobs.length > 0) {
    expect(parsed.jobs[0]).toHaveProperty("title")
    expect(parsed.jobs[0].source).toBe("stepstone")
  }
}, 90000)
