...
test("detail is unsupported for glassdoor", async () => {
  const { stderr, exitCode } = await runCLI(["detail", "https://www.glassdoor.com/job-listing/x"])
  expect(exitCode).toBe(1)
  expect(JSON.parse(stderr).code).toBe("UNSUPPORTED")
})

const live = process.env.APIFY_TOKEN ? test : test.skip
live("live: search returns normalized jobs", async () => {
  const { stdout, exitCode } = await runCLI([
    "search", "--query", "product manager", "--location", "Berlin, Germany", "--limit", "3", "--format", "json",
  ])
  expect(exitCode).toBe(0)
  const parsed = JSON.parse(stdout)
  expect(parsed.meta.source).toBe("glassdoor")
  if (parsed.jobs.length > 0) expect(parsed.jobs[0].source).toBe("glassdoor")
}, 90000)
