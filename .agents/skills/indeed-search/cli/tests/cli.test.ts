...
live("live: search returns normalized jobs (DE)", async () => {
  const { stdout, exitCode } = await runCLI([
    "search", "--query", "software engineer", "--location", "Berlin", "--limit", "3", "--format", "json",
  ])
  expect(exitCode).toBe(0)
  const parsed = JSON.parse(stdout)
  expect(parsed.meta.source).toBe("indeed")
  expect(parsed.meta.country).toBe("DE")
  if (parsed.jobs.length > 0) expect(parsed.jobs[0].source).toBe("indeed")
}, 90000)
